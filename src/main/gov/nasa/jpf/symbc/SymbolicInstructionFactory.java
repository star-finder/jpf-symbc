...
public class SymbolicInstructionFactory extends gov.nasa.jpf.jvm.bytecode.InstructionFactory {
  ...
  public static Logger LOGGER = JPFLogger.getLogger(SymbolicInstructionFactory.class.getName());
	 ...
	 */
	private boolean pcChoiceOptimization;

  /*
   * With this setting, incremental solving is used
  */
  public static boolean incrementalSolving;
	 ...
	 public  SymbolicInstructionFactory (Config conf){
      ...
      startSystemMillis = System.currentTimeMillis();
		...
		if (regress != null && regress.equals("true")) {
		  ...
		  //I cannot believe we have to do this. It does not seem there is any way of obtaining the JPF
     // instance and check if the listener has been added.
     //It is crazy how much static state is maintained.
    incrementalSolving = false;
     ...
     if(listeners != null) {
       for (String listener : listeners) {
		...
		//When incremental solving is on *DONT* use CG optimization
     if(incrementalSolving && pcChoiceOptimization) {
       LOGGER.warning("Incremental solving cannot be used with pc choice optimization at the " +
           "moment. Disabling pc choice optimization");
       this.pcChoiceOptimization = false;
     }
